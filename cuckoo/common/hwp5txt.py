...
class HwpFile(object):
	...
	def get_text_from_stream(self, payload):
		regex = re.compile(r'([\x00-\x1f])\x00')
		text = ''
	...
	def get_text(self):
		stream = self.ole.openstream('BodyText/Section0')
		...
		while True:
			...
			if tag_name == 'HWPTAG_PARA_TEXT':
				#print self.get_text_from_stream(payload)
				text += self.get_text_from_stream(payload)
			'''TABLE Parsing
